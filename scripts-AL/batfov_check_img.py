...
img_file = fits.open(filename)

img_data = img_file[0].data

## read in WCS info from fits header
## http://docs.astropy.org/en/stable/wcs/
w = wcs.WCS(img_file[0].header)

number_x = len(img_data)
number_y = len(img_data[1])
pcode_array = np.zeros((number_x, number_y), np.float)

total_pcode = 0
for i in range(len(img_data)):
        for j in range(len(img_data[1])):
		if 'nan' not in str(img_data[i][j]):
			radec = w.wcs_pix2world(j,i,1)
