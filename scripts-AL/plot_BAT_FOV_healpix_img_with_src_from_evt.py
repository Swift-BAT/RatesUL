...
## get input source list from event detection list
detection_source_list = open(output_dir + '/event_data_analysis/' + obsid + '_image_search/detect_source_list.txt')

input_src_RA = []
input_src_DEC = []
for line in detection_source_list.readlines():
	if "UNKNOWN" in line:
		column = line.split()
		src_RA = float(column[3])
		src_DEC = float(column[4])
		src_SNR = float(column[7])
		print src_RA, src_DEC, src_SNR
		if (src_SNR >= 5.0):
			input_src_RA.append(src_RA)
			input_src_DEC.append(src_DEC)
	
detection_source_list.close
...
annim.Marker(x=healpix_x, y=healpix_y, mode='world', marker='o', color='b', markeredgecolor='b', markersize=0.01)
annim.Marker(x=healpix_x_med, y=healpix_y_med, mode='world', marker='o', color='skyblue', markeredgecolor='skyblue', markersize=0.05)
annim.Marker(x=healpix_x_med_1, y=healpix_y_med_1, mode='world', marker='o', color='lightblue', markeredgecolor='lightblue', markersize=0.08)
annim.Marker(x=healpix_x_hi, y=healpix_y_hi, mode='world', marker='o', color='aliceblue', markeredgecolor='aliceblue', markersize=0.1)
...
src_SNR_array = ["test"]
#annim.text(x=input_src_RA, y=input_src_DEC, src_SNR_array)
#fig.text = (0.0, 0.0, "test")

annim.plot()
#annim.interact_toolbarinfo()
#annim_rep.interact_imagecolors()
#show()
outfile = output_dir + "/bat_fov_" + obsid + ".png"
savefig(outfile)
