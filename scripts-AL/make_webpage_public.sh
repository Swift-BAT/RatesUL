...
## This script is basically the same as make_webpage.sh, but we remove some info that we don't want the public to see (e.g. GCN template...)
...
set webpage = ${output_dir}/web/source_public.html
...
## copy BAT FOV image
/bin/cp ${output_dir}/bat_fov.png ${web_dir}

## raw light curve
if (${pass_data_flag} == 0) then
	echo "No pass data yet" >> ${webpage}
else
