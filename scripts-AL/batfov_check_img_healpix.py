...
Earth_RA = sys.argv[5]
Earth_DEC = sys.argv[6]
#prob_min = float(sys.argv[7])
...
prob_in_BAT_FOV = 0
prob_tot = 0
prob_out_BAT_FOV_not_in_Earth_limb = 0
prob_out_BAT_FOV_rest = 0
	...
	if (pcode >= 0.1):
		...
		if ('N/A' not in str(Earth_RA)):
			if (dist_from_Earth > 69.0):
				prob_out_BAT_FOV_not_in_Earth_limb += prob
			else:
				prob_out_BAT_FOV_rest += prob
		else:
			prob_out_BAT_FOV_not_in_Earth_limb = 'N/A'

	## calculate pcode for the region of prob > prob_min
#	if (prob >= prob_min):			
#		i_healpix_tot += 1
#		total_pcode += pcode	
#		## calculate the percentage of LIGO map that are covers by BAT FOV (with 0.1)
#		if (pcode >= 0.1):
#			i_healpix += 1	


#frac_LIGO_in_BAT = float(i_healpix)/float(npix) ## I think this is not a good fraction to represent the LIGO fraction in BAT
#frac_LIGO_in_BAT = float(i_healpix)/float(i_healpix_tot) ## This is also not a new estimation of LIGO prob fraction

#frac_LIGO_test = float(i_healpix_tot)/float(npix)
...
#print 'input_img_FOV_flag:', fov_flag
print 'input_img_pcode_prob:', total_pcode_prob
#print 'frac_LIGO_map_in_BAT_FOV:', frac_LIGO_in_BAT
#print 'TEST: frac_LIGO_test:', frac_LIGO_test
print 'LIGO_prob_in_BAT_FOV:', prob_in_BAT_FOV
print 'TEST: prob_tot', prob_tot
print 'prob_out_BAT_FOV_not_in_Earth_limb:', prob_out_BAT_FOV_not_in_Earth_limb
print 'TEST: prob_out_BAT_FOV_rest', prob_out_BAT_FOV_rest
