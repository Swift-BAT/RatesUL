...
#filename = home + '/' + obsid + '/bat/rate/sw' + obsid + 'brtqd.lc.gz'
filename = home + '/quad_rate_sort_test.lc'
...
for i in range(len(lc_fits_data)):
	...
	if(xmin <= lc_fits_data[i][0]-T0 <= xmax):
		...
		time.append(lc_fits_data[i][0]-T0+1.6) ## create fake 1.6s bin plot
...
#plt.savefig(home + '/quad_rate_lc.png',bbox_inches='tight')
...
print 'flag_quad_rate:', flag_quad_rate
