...
set T0_10s_after = `cat ${info_file} | grep "T0_10s_after:" | awk '{printf("%lf\n", $2)}'`
...
set BAT_RA_10s_after = `cat ${info_file} | grep "BAT_RA_10s_after:" | awk '{printf("%lf\n", $2)}'`
set BAT_DEC_10s_after = `cat ${info_file} | grep "BAT_DEC_10s_after:" | awk '{printf("%lf\n", $2)}'`
set BAT_ROLL_10s_after = `cat ${info_file} | grep "BAT_ROLL_10s_after:" | awk '{printf("%lf\n", $2)}'`
...
python batfov_mod.py ${BAT_RA_10s_after} ${BAT_DEC_10s_after} ${BAT_ROLL_10s_after} ${output_dir}
...
if ($input_img != 'NONE') then
	...
	if (${healpy_flag} == 0) then
		echo "python plot_BAT_FOV_healpix_img_with_src.py ${BAT_FOV_img} ${input_img} ${src_RA} ${src_DEC} ${output_dir}"
                python plot_BAT_FOV_healpix_img_with_src.py ${BAT_FOV_img} ${input_img} ${src_RA} ${src_DEC} ${output_dir} ${T0_10s_after} >> ${info_file}
        endif
